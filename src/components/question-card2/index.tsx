...
const QuestionCard: React.FC<QuestionCardProps> = ({ question, answer }) => {
  ...
  >
    <div
      className={tw(`w-full shadow-base rounded-xl border border-border-white flex-col justify-start lg:px-8 sm:px-4 md:px-6 gap-4 inline-flex mb-4 ${isOpen ? 'bg-green-dark' : 'bg-green'}`)}
    >
      <div
      ...
      {isOpen && (
        ...
        <H2 className={tw('text-left mb-6 sm:m-4 text-white-light')}>{answer}</H2>
        </div>
      )}
    </div>
    </motion.div>
