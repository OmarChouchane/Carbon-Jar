...
const BigCard2: React.FC<BigCardProps> = ({
  ...
  <div
    className="flex lg:mt-12 flex-col bg-light-green mx-auto lg:flex-row justify-start items-left lg:items-stretch lg:p-8 p-2 rounded-lg border border-lighter-green"
  >
    <div className="flex flex-col justify-start items-left gap-6 lg:ml-6">
      <div className="flex flex-col justify-start items-left">
        <SmallerH1 className="mx-10">{title}</SmallerH1>
        <br />
        {description1 && <H2 className="text-left">{description1}</H2>}

        <div className="flex items-left mx-6">
          {icon && <img src={icon} alt="icon" className="w-6 h-6" />}
          <H2 className="font-bold justify-start">{Maindescription}</H2>
        </div>
        <H2 className="lg:mx-6  mx-2 justify-start text-left">
          {description}
        </H2>
      </div>
      {button1 && (
