...
const QuestionCard: React.FC<QuestionCardProps> = ({ question, answer }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}>
      <div
        className={`w-full shadow-base rounded-xl border border-lighter-green flex-col justify-start lg:px-8 sm:px-4 md:px-6 gap-4 inline-flex mb-4 ${isOpen ? "bg-lighter-grey" : "bg-white"}`}
      >
        <div
        ...
        {isOpen && (
          <div className="pl-4 pr-6 pt-4 pb-4 text-left transition-all duration-300 ease-in-out sm:pl-6 sm:pr-12 lg:pl-10 lg:pr-28">
            <H2 className="text-left mb-6 sm:m-4 ">{answer}</H2>
          </div>
        )}
      </div>
    </motion.div>
